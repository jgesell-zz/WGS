...
mkdir -p ${dir}/Logs;
mkdir -p ${dir}/Results;
echo "/gpfs1/projects/mcwong/work/bbmap/bbduk.sh in1=${i} out1=\${TMPDIR}/${base}.1.Trimmed.fq ref=\${TMPDIR}/Adapters.fasta k=17 hdist=1 minlength=50 qtrim=rl trimq=20 tbo=t tpe=t strictoverlap=f minavgquality=20 maxns=0 entropywindow=20 entropyk=5 mink=10 ktrim=r stats=${dir}/Logs/${base}.TrimmingStats.txt threads=8 minlength=50 overwrite=t qout=33 entropy=.70 1>${dir}/Logs/${base}_trimming_report.txt 2>${dir}/Logs/${base}_trimming_report.Error.txt; bowtie2 -x \${TMPDIR}/hg38phix -U \${TMPDIR}/${base}.1.Trimmed.fq --end-to-end --very-sensitive -p 8 --no-unal --no-hd --mm --no-sq 2> ${dir}/Logs/${base}.hg38phix.align.stats.txt | cut -f1,3 > \${TMPDIR}/${base}.hg38phix.reads.txt; perl /users/mcwong/fastqFilterSingle.pl \${TMPDIR}/${base}.1.Trimmed.fq ${outdir}/${base}_1.fastq.bz2 \${TMPDIR}/${base}.hg38phix.reads.txt; lbzip2 -n8 -c \${TMPDIR}/${base}.hg38phix.reads.txt > ${outdir}/${base}.hg38phix.reads.txt.bz2; rm \${TMPDIR}/${base}.*;";done | shuf | shuf > ../Scripts/TrimCommand.txt;
